...
export class JsonDataService<T extends Identifiable<K>, K> implements DataService<T, K> {
	...
	update(key: K, data: Partial<T>): boolean {
		const dataArray = this.retrieveAll();
		const indexOf = dataArray.findIndex((d) => d.id === key);
		if (indexOf >= 0) {
	...
	delete(key: K): boolean {
		const dataArray = this.retrieveAll();
		const filteredData = dataArray.filter((d) => d.id !== key);
